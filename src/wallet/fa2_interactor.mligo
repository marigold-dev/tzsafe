...
let create_token ((fa2, md) : f_storage * FA2.TZIP12.tokenMetadataData) : f_storage =
  let () = assert_with_error (not (Big_map.mem md.token_id fa2.token_metadata)) "Token is already exist" in
  let new_metadata = Big_map.add md.token_id md fa2.token_metadata in
  { fa2 with token_metadata = new_metadata }